...
_p[48] = {
     ...
     */
        var ControlHandle = kity.createClass("ControlHandle", {
            ...
            constructor: function() {
                this.callBase();
                // 控制点圆点
                this.dot = new kity.Circle(6).fill("#fff").stroke("#4285f4", 2);
                // 连接线（从锚点到控制点）
                this.line = new kity.Path().stroke("#999", 1).setPathData([ "M", 0, 0, "L", 0, 0 ]);
     ...
     */
        var TempConnection = kity.createClass("TempConnection", {
            ...
            constructor: function(minder, sourceNode) {
                ...
                // 当前悬停的目标节点
                // 创建临时连线路径
                this.path = new kity.Path().stroke("#4285f4", 2).setOpacity(.6);
            ...
            updateEndPoint: function(x, y) {
                ...
                var dy = y - start.y;
                var cx = start.x + dx / 2;
                var pathData = [ "M", start.x, start.y, "Q", cx, start.y, x, y ];
            ...
            updateToNode: function(targetNode) {
                ...
                var cx = start.x + dx / 2;
                var pathData = [ "M", start.x, start.y, "Q", cx, start.y, end.x, end.y ];
                this.path.setPathData(pathData);
            },
            remove: function() {
     ...
     */
        var HyperConnection = kity.createClass("HyperConnection", {
            ...
            constructor: function(minder, data) {
                ...
                // 创建透明的点击热区（更宽，但不可见）
                this.hitArea = new kity.Path().stroke("transparent", 12).fill("none");
            ...
            createArrowMarker: function() {
                ...
                var arrow = new kity.Path().setPathData("M0,0 L8,4 L0,8 L2,4 Z").fill(this.data.color || "#666");
            ...
            select: function() {
                ...
                this.minder.fire("hyperconnectionselect", {
                    connection: this
                });
         ...
         */
            ...
            update: function() {
                ...
                if (this.textShape) {
                    ...
                    var mt = 1 - t;
                    var midPoint = {
                        x: mt * mt * mt * start.x + 3 * mt * mt * t * controlPoints.cp1.x + 3 * mt * t * t * controlPoints.cp2.x + t * t * t * end.x,
                        y: mt * mt * mt * start.y + 3 * mt * mt * t * controlPoints.cp1.y + 3 * mt * t * t * controlPoints.cp2.y + t * t * t * end.y
                    };
         ...
         */
            calculateControlPoints: function(start, end) {
                ...
                var controlFactor = .3;
                // 控制点沿连线方向的偏移比例
                var curvature = Math.min(30, dist * .15);
                // 默认弧度，根据距离自适应
                // 计算垂直于连线方向的单位向量
                var perpX = dy / dist;
                var perpY = -dx / dist;
                ...
                // 应用用户的偏移量
                var offset1 = this.data.controlPoint1 || {
                    x: 0,
                    y: 0
                };
                var offset2 = this.data.controlPoint2 || {
                    x: 0,
                    y: 0
                };
                return {
         ...
         */
            drawPath: function(start, end, controlPoints) {
                var pathData = [ "M", start.x, start.y, "C", controlPoints.cp1.x, controlPoints.cp1.y, controlPoints.cp2.x, controlPoints.cp2.y, end.x, end.y ];
                // 透明热区和可见路径使用相同的路径数据
     ...
     */
        Module.register("HyperConnection", {
            init: function() {
                // 创建超连接容器，放在普通连线之上
                this._hyperConnectionContainer = new kity.Group().setId(utils.uuid("minder_hyperconnection_group"));
            ...
            events: {
                ...
                "hyperconnection.mousemove": function(e) {
                    ...
                    if (minder._hoveredNode) {
                        ...
                        // 添加一些容差，避免在节点边缘抖动
                        var margin = 10;
                        var inBox = pos.x >= hoveredBox.x - margin && pos.x <= hoveredBox.right + margin && pos.y >= hoveredBox.y - margin && pos.y <= hoveredBox.bottom + margin;
                        if (inBox) {
                            // 还在节点范围内，继续吸附
                    ...
                    if (targetNode && targetNode !== minder._connectionSourceNode) {
                        ...
                        targetNode.getRenderContainer().setOpacity(.7);
